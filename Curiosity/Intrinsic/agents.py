...
class RNDAgent(object):
    ...
    def get_action(self, state):
        ...
        state = state.float()
        policy, value_ext, value_int, f = self.model(state)
        action_prob = F.softmax(policy, dim=-1).data.cpu().numpy()

        action = self.random_choice_prob_index(action_prob)

        return action, value_ext.data.cpu().numpy().squeeze(), value_int.data.cpu().numpy().squeeze(), policy.detach(), f
    ...
    def train_model(self, s_batch, target_ext_batch, target_int_batch, y_batch, adv_batch, next_obs_batch, old_policy):
        ...
        for i in range(self.epoch):
            ...
            for j in range(int(len(s_batch) / self.batch_size)):
                ...
                policy, value_ext, value_int, _ = self.model(s_batch[sample_idx])
