function plot_dist2(eps){
    d3.selectAll("#ep_dist2 svg").remove();
    ...
    d3.text("actions_final_int2.csv", function(text) {
        ...
        d3.text("rewards_final_int2.csv", function(text) {
            ...
            var width = d3.select('#ep_dist2').node().getBoundingClientRect().width * (94/100);
            var height = d3.select('#ep_dist2').node().getBoundingClientRect().height * (94/100);
            var margin = d3.select('#ep_dist2').node().getBoundingClientRect().width * (10/100);
            var margin_y = d3.select('#ep_dist2').node().getBoundingClientRect().height * (7/100);

            var svg_container1 = d3.select("#ep_dist2").append("svg")
            ...
            // actions = [{"action":"fire"},{"action":"left"},{"action":"right"}];
            // svg_container1.selectAll('rct')
            // .data(actions)
            // .enter()
            // .append("rect")
            // .attr("x", function(d, i){ return (20*4*i)})
            // .attr("y", -20)
            // .attr("width", "10px")
            // .attr("height", "10px")
            // .style("fill", function (d) { return color_legend(d.action); } )
            
            // svg_container1.selectAll('txt')
            // .data(actions)
            // .enter()
            // .append("text")
            // .attr("x", function(d, i){ return (20*4*i);})
